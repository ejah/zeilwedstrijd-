# File utils created on: 11-05-16 by: ejah
...
class ActivationMailFormMixin:
    ...
    def set_mail_sent(self, value):
        raise TypeError('Can not sent mail_sent attribute.')
    ...
    def send_mail(self, user, **kwargs):
        ...
        if request is None:
            ...
            logger.warning(
                'send mail called without request.\n'
                'Traceback:\n'.format(''.join(tb))
            )
    ...
    def log_mail_error(self, **kwargs):
        ...
        for recipient in recipient_list:
            msg_list.insert(1, 'recipient: {r}'.format(r=recipient))
        if 'error' in kwargs:
            level = logging.ERROR
            error_msg = (
